...
> ℹ️ **小米平板 5 / 5 Pro 等设备通常使用 `ext4` 文件系统**，构建模块时建议将参数 `fs` 设置为 `ext4`，以确保兼容性和构建成功率。  
...
| `fs`     | `ext4`（推荐米板5） / `erofs`（默认）         | 镜像解压方式，建议根据设备实际情况设置       |

> 💡 如果你使用的是 **小米平板 5 / 5 Pro**，请将 `fs` 设置为 `ext4`，否则可能因格式不符导致构建失败。
